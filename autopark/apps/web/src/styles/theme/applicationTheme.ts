...
const applicationTheme = (color, mode, direction): any => ({
  direction,
  palette: {
    mode: mode,
    ...
    background: { default: mode == 'dark' ? '#303030' : 'rgb(247, 252, 254)' },
  },
  typography: {
  ...
  components: {
    ...
    MuiInput: {
      styleOverrides: {
        root: {
          border:
            mode === 'dark' ? '1px solid rgba(255,255,255,0.32)' : '1px solid rgba(0,0,0,0.32)',
          borderRadius: 8,
          overflow: 'hidden',
          alignItems: 'center',
